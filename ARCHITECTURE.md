...
An opt-in illustration prototype can assemble validated bitmap and SVG assets
into deterministic SVG scenes with explicit z-order, transforms, typography,
asset-root containment, and per-source hashes. The canonical
`illustrate-preschool-story` skill defines interaction-aware hybrid assembly:
reuse safe independent layers, keep buried or occluded props in anchored plates,
keep physical contact in coherent interaction modules, and retain bounded
full-page raster exceptions. A second opt-in prototype compiles normalized
character-rig landmarks, page anchors, and action specifications into
page-space geometry and deterministic Rapier 2D static pose proofs for buried
depth, jointed flexible stalks,
crown-to-grip-to-leaf-tip topology, contact continuity, body counterbalance,
and ground support. Its machine-readable proof controls geometry supplied to
generation but does not approve the resulting raster; visual contact must
still be reviewed. These prototypes are not called by `BookProductionService`
or any parent-facing route. A provider-neutral `IllustrationProductionPlan`
compiler now validates exact artifact lineage, compiles every page into an
ordered work request, requires the Rapier gate only for buried geometry,
flexible tension, or long contact chains, and selects layered, hybrid, or
documented integrated-page composition. It also makes one-at-a-time missing
unit generation, deterministic typography, quiet contrast, bounded emotional
performance, rendered-raster inspection, and specialist review explicit. The
developer CLI persists the compiled plan, per-page work requests, and checklist
to a caller-selected local directory without invoking a paid provider. Wiring
this plan into `BookProductionService` or a parent-facing route still requires
an explicit lifecycle, staleness, cost, and approval design.
...
| 2026-09-21 | Added a rig + scene-anchor + action compiler and plant-topology validation to the opt-in Rapier illustration proof, then used its coordinates to assemble a one-page hybrid proof without changing the approved character or background assets. | `src/lib/illustration-physics/`, `scripts/render-turnip-final-pipeline-poc.mjs`, `.agents/skills/illustrate-preschool-story/`, `tasks/turnip-style-preserved-hybrid-pilot.md` |
| 2026-09-22 | Added a provider-neutral illustration production compiler that turns exact approved-artifact revisions into interaction-aware page work orders with conditional Rapier, contrast, emotion, typography, and review gates.                        | `src/lib/illustration-pipeline/`, `scripts/compile-illustration-production-plan.mjs`, `tasks/book-illustration-production-pipeline.md`                                        |
