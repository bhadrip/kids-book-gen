...
| `ProjectBrief` | Preserves the original idea, protagonist hints, desire, structured story mood or custom mood, meaning, avoid list, must-keep details, and confirmed reader age/reading mode. Legacy briefs remain readable without the newer structured fields but cannot start new text generation without reader details. | Project and parent intake | Parent-authored | `brief.json` |
