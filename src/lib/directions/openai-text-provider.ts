  ...
  public async generateStory(
    ...
    const response = await new OpenAI({ apiKey: this.apiKey }).responses.parse({
      ...
      input: [
        {
          role: "developer",
          content: `Write a safe story package tuned to this reader profile: ${readerProfileGuidance(brief.readerConfiguration)} ${storyMoodInstruction(brief)} Treat the mood as a required creative constraint in every spread and in the ending; do not silently replace it with a generic warm tone. ${storyThemeInstruction(brief)} Make the selected idea materially shape conflict, choices, consequences, and resolution while avoiding a lecture. Preserve must-keep details. Return exactly 13 concise, non-empty spreads suited to the declared reading mode.`,
        },
        {
          role: "user",
          content: JSON.stringify({
  ...
  public async evaluateStory(
    ...
    const response = await new OpenAI({ apiKey: this.apiKey }).responses.parse({
      ...
      input: [
        {
          role: "developer",
          content: `Privately evaluate this children's story for fidelity to the family brief, causal structure, reader fit, delivery-mode fit, mood fidelity, theme enactment, and safety. Apply this versioned reader profile materially—not as a generic age label: ${readerProfileGuidance(brief.readerConfiguration)} ${storyMoodInstruction(brief)} Mark moodFit revise when the manuscript does not materially sustain the requested tone, pacing, emotional intensity, dialogue, suspense, or closure. ${storyThemeInstruction(brief)} Mark themeFit revise when the selected idea does not materially shape conflict, choices, consequences, recognition, or resolution, or when it appears mainly as a lecture or stated moral. Check causal complexity, inference support, protagonist agency, vocabulary support, text density, referential clarity, participation opportunities, emotional intensity, and oral flow or decoding demand as appropriate. Choose revise only for a material problem. If revision is needed, provide one concise revision brief that preserves strengths and approved details. Return only the structured response.`,
        },
        {
          role: "user",
