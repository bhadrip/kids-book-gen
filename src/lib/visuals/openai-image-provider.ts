...
export class OpenAIImageProvider implements ImageProvider {
  ...
  public async generateCharacterDesigns(
    ...
    const prompt = [
      ...
      `Whole-story emotional performance requirements: ${JSON.stringify(input.emotionalArc)}`,
      `Approved visual spread plan: ${JSON.stringify(input.spreadMap)}`,
  ...
  public async generateSampleSpread(
    ...
    const prompt = [
      ...
      `Approved emotional performance: ${JSON.stringify(input.emotionalArc)}`,
      `Approved spread plan: ${JSON.stringify(input.spreadMap.spreads.find((candidate) => candidate.spreadNumber === spread.number))}`,
