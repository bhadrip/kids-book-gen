...
| ID | Task | Status | Evidence / notes |
| --- | --- | --- | --- |
| DOM-01 | Define versioned Zod schemas for projects, briefs, directions, story packages, visual bibles, spreads, proofs, jobs, and feedback. | **not started** | |
...
| DOM-03 | Implement injectable clocks and ID generation for deterministic tests. | **not started** | |
| STO-01 | Implement a file-backed project repository at `data/projects/<project-id>/`. | **not started** | |
| STO-02 | Implement atomic JSON/artifact writes and validated reads. | **not started** | |
| STO-03 | Implement project creation, loading, and restart-resume behavior. | **not started** | |
| JOB-01 | Implement a file-backed local job runner with persisted progress and safe recovery. | **not started** | |
| TST-01 | Test schema validation, lifecycle/staleness, persistence, and restart-resume behavior. | **not started** | |
